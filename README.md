...
같은 목록이 여러 저장소에서 따로 관리되면 한쪽만 고쳤을 때 내용이 어긋납니다. 목록을 고치는 곳을 여기 하나로 두고, 다른 저장소는 여기서 만들어 낸 사본을 씁니다.
...
| 칸 | 뜻 |
|---|---|
| `banned` | 찾을 글자. 소비자는 이 글자가 들어 있으면 걸리도록 찾습니다 |
...
| `scope` | `answer` 는 답, `artifact` 는 산출물 문서, `living-doc` 은 저장소의 살아 있는 문서입니다 |
| `enabled` | `false` 면 검사에 걸지 않습니다. 지우지 않고 `disabled_reason` 과 함께 남깁니다 |
| `evidence` | `user`·`measured`·`reasoned`·`none` 넷. 앞의 둘에는 날짜와 원문이 따라붙습니다 |
| `rules` | 금지어와 대체어 쌍으로 표현되지 않는 규칙. 단어를 찾는 소비자는 건너뜁니다 |

어간을 적을 때 한글은 음절이 한 글자입니다. `맞대` 가 `맞대고` 는 품지만 `맞댄` 은 품지 않으므로 어간을 여럿 적습니다. 반대로 다른 단어에 나타나는 어간은 뺍니다. `가르다` 에 `가르` 를 넣으면 `가르치다` 가 걸립니다.
