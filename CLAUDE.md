...
생성물의 앞 두 칸 순서를 바꾸지 않고 칸을 뒤에 추가하는 것만 허용하므로, `schema` 가 올라가도 옛 소비자가 그대로 읽습니다. 소비자는 `schema` 를 정수로 `updated` 를 문자열로 견주어 최신을 가립니다. 둘이 같은데 버전 표시의 셋째 값(내용 지문)이 다르면 어느 쪽이 새것인지 알 수 없으므로 건드리지 말고 알려야 합니다. `updated` 가 날짜뿐이라 하루에 여러 번 고치면 값이 같기 때문입니다.
