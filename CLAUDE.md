...
생성물은 매 세션 모든 PC의 `CLAUDE.md` 에 실립니다. 행이 늘면 그만큼이 매번 컨텍스트 비용이 됩니다. 2026-09-20 에 3,462자였던 것이 사흘 만에 5,097자가 되었습니다.

**기준선은 만자당 0.15 입니다.** 그 항목이 실제로 적용되는 말뭉치에서 셉니다. 적용 대상이 `답변과 산출물` 이면 Claude 답변에서 세고, `문서와 답변` 이면 문서까지 합쳐 셉니다. 전체 말뭉치로 세면 답변에만 나타나는 구어가 묻힙니다.

**`evidence.kind` 가 `user` 인 항목은 빈도와 무관하게 유지합니다.** 사용자가 직접 요구한 것이라 빈도로 뺄 수 없습니다. `기대를 걸` 은 379만 자에서 1건이지만 유지합니다.

기준선 미만이면 지우지 말고 `enabled` 를 `false` 로 바꾸고 `disabled_reason` 에 측정 날짜와 건수를 적습니다. 넣을지 검토했다가 기각한 후보도 같은 방식으로 꺼진 항목에 넣습니다. 그러면 무엇을 왜 뺐는지가 데이터 한 곳에 남아 몇 달 뒤 같은 논의를 다시 하지 않습니다.

`render.py` 가 꺼진 항목을 표에서 빼고 맨 아래 한 줄로만 이름을 적습니다. 소비자는 그 낱말을 검사하지 않습니다.

빈도 분포는 한쪽으로 쏠려 있습니다. 2026-09-23 측정에서 상위 20개 항목이 전체 검출의 81.4% 를 냈습니다. 기준선을 올려도 검출은 거의 안 줄고 표만 짧아집니다.
